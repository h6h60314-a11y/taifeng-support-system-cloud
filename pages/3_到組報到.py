...
from db import get_arrivals, get_request_by_no, get_request_options, init_db, insert_arrival
...
with st.form("arrival_form", clear_on_submit=True):
    ...
    with col2:
        ...
        arrival_date = st.date_input("到組日期", value=datetime.now().date())
        arrival_time_input = st.time_input("到組時間", value=datetime.now().time())
