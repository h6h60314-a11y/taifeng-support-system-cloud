...
from db import get_departures, get_request_by_no, get_request_options, init_db, insert_departure, now_tw
...
with st.form("departure_form", clear_on_submit=True):
    ...
    with col2:
        ...
        depart_date = st.date_input("離組日期", value=now.date())
        depart_time_input = st.time_input("離組時間", value=now.time().replace(microsecond=0))
