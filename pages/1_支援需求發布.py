...
from db import generate_request_no, get_requests, init_db, insert_support_request, now_tw
...
with st.form("support_request_form", clear_on_submit=True):
    ...
    with col1:
        ...
        publish_date = st.date_input("發布日期", value=now.date())
        required_count = st.number_input("需求人數", min_value=1, max_value=50, value=1, step=1)

    with col2:
        publish_time_input = st.time_input("發布時間", value=now.time().replace(microsecond=0))
...
if not filtered_df.empty:
    ...
    if keyword.strip():
        mask = (
            ...
            | filtered_df["reason"].astype(str).str.contains(keyword, case=False, na=False)
            | filtered_df["note"].astype(str).str.contains(keyword, case=False, na=False)
        )
