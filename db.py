...
def insert_departure(name: str, origin_team: str, target_team: str, depart_time: str, request_no: str, status: str) -> None:
    ...
    with get_conn() as conn:
        ...
        conn.execute(text("UPDATE support_requests SET status = '支援中', updated_at = :updated_at WHERE request_no = :request_no AND status = '待支援'"), {
            "updated_at": ts,
            "request_no": request_no,
        })
...
def auto_update_request_status(request_no: str, conn=None) -> None:
    ...
    if conn is None:
        ...
        cm = get_conn()
        conn = cm.__enter__()
    req = conn.execute(text("SELECT required_count FROM support_requests WHERE request_no = :request_no"), {"request_no": request_no}).fetchone()
    if req:
        arrived_count = conn.execute(text("SELECT COUNT(*) FROM arrivals WHERE request_no = :request_no"), {"request_no": request_no}).scalar_one()
        new_status = "已補足" if arrived_count >= req[0] else "支援中"
        conn.execute(text("UPDATE support_requests SET status = :status, updated_at = :updated_at WHERE request_no = :request_no"), {
            "status": new_status,
            "updated_at": now_str(),
            "request_no": request_no,
        })
...
def get_dashboard_data() -> dict[str, Any]:
    today = datetime.now().strftime("%Y-%m-%d")
    requests_df = fetch_df("SELECT * FROM support_requests WHERE substr(publish_time, 1, 10) = :today ORDER BY publish_time DESC", {"today": today})
    departures_df = get_departures()
    arrivals_df = get_arrivals()
    if departures_df.empty:
        pending_arrival_df = departures_df.copy()
    else:
        merged = departures_df.merge(arrivals_df[["name", "request_no", "arrival_time"]], on=["name", "request_no"], how="left")
